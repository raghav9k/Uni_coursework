...
void swap(struct node* ref1, struct node* ref2) {
    ...
    if(ref1->next == ref2){
        int temp = value(ref2);
        insert(prev(ref1),temp);
        erase(ref2);
        return;
        
    }if(ref2->next==ref1){
        int temp = value(ref1);
        insert(prev(ref2),temp);
        erase(ref1);
        return;
    }
    else{
        
        struct node* bef_ref1 = ref1->prev;
        struct node* bef_ref2 = ref2->prev;
        struct node* aft_ref1 = ref1->next;
        struct node* aft_ref2 = ref2->next;

        // Relink before and after nodes
        if( bef_ref1 ) bef_ref1->next = ref2;
        if( bef_ref2 ) bef_ref2->next = ref1;
        if( aft_ref1 ) aft_ref1->prev = ref2;
        if( aft_ref2 ) aft_ref2->prev = ref1;
...
struct node* concat(struct node* ref1, struct node* ref2) {
    /* Your code here */
    struct node* end1 = end(ref1);
    struct node* beg2 = begin(ref2);
    end1->next = beg2;
    beg2->prev = end1;
    return begin(ref1);
}
...
int distance(struct node* ref1, struct node* ref2) {
    ...
    int dist = 0;
    if( begin(ref1) == ref1 && end(ref2)==ref2){
        return size(ref1)-1;
    }
    struct node* ptr1 = ref1;
    struct node* ptr2 = ref1;
    while(value(ptr1)!=value(ref2)&& value(ptr2)!= value(ref2)){
        while(ptr1->next != NULL){
            ptr1 = ptr1->next;
            dist = dist + 1;
            if(value(ptr1)==value(ref2)){
                return dist;
            }
        }
        dist = 0;
...
struct node* unique(struct node* ref) {
    ...
    while(ptr1->next != NULL){
        ptr2 = ptr1->next;
        if(value(ptr1) == value(ptr2)){
            struct node* del = ptr2;
            ptr2 = erase(del);
            ptr1 = ptr1->prev;
        }
        ptr1 = ptr1->next;
